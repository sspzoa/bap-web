...
export const MealNavigationBar = memo(function MealNavigationBar({
  ...
  return (
    <div className="flex flex-row gap-4 px-4 md:px-0">
      <Glass
        className="active:scale-95 active:opacity-50 duration-100 shrink-0 w-[54px] h-[54px] flex justify-center items-center cursor-pointer order-1"
        onClick={onPrevDay}>
        <Image src="/icon/arrow-left.svg" alt="arrow-left" width={32} height={32} draggable={false} />
      </Glass>

      <Glass
        className="flex justify-center items-center w-full h-full cursor-pointer active:scale-95 active:opacity-50 duration-100 order-2"
        {...longPressProps}>
        <p className="text-xl md:text-[22px] font-bold tracking-tight">{formattedCurrentDate}</p>
      </Glass>

      <Glass
        className="active:scale-95 active:opacity-50 duration-100 shrink-0 w-[54px] h-[54px] flex justify-center items-center cursor-pointer order-3"
        onClick={onNextDay}>
